...
import { getPlaylistDetails, getPlaylistRecommendations } from "../services/api";
...
const PlaylistDetails = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [playlistData, recData] = await Promise.allSettled([
          getPlaylistDetails(id),
          getPlaylistRecommendations(id)
        ]);
        
        if (playlistData.status === "fulfilled") {
          setPlaylist(playlistData.value);
        } else {
          console.error("Failed to fetch playlist:", playlistData.reason);
        }
  ...
  if (!playlist) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh]">
        <p className="text-dark-300 text-lg">Playlist not found</p>
      </div>
    );
  }
    ...
    <div className="fade-in">
          ...
          <div className="flex flex-col justify-end text-center md:text-left">
            ...
            {playlist.headerDesc && (
              <p className="text-sm text-dark-200 mb-2 max-w-lg">
                {cleanTitle(playlist.headerDesc)}
              </p>
            )}
      ...
      {recommendations && recommendations.length > 0 && (
        <div className="mt-12">
          ...
          <Carousel>
            {recommendations.map((recPlaylist) => (
              <SongCard key={recPlaylist.id} song={recPlaylist} />
            ))}
          </Carousel>
        </div>
      )}
